...
library CustomStun initializer OnMapLoad
    ...
    globals
        ...
        private constant integer SYSTEM_DUMMY_ID   = 'h001'
    ...
    struct CustomStun extends array
        ...
        static method AddUnitStunRemaining takes unit dst, real delta returns boolean
            ...
            return true
        endmethod
        /* 커스텀 스턴 - (dst)유닛을 (timeout)초 동안 기절시킵니다, 만약 적용중인 스턴이 더 길거나 스턴 면역이면 무시합니다.*/
        static method Stun2 takes unit dst, real timeout returns nothing
            if GetUnitTypeId(dst) == 0 then
                return
            endif
            if GetUnitAbilityLevel(dst,'A00Q') < 1 then
                if GetUnitStunRemaining(dst) < timeout then
                    call Clear(dst)
                    set T_SRC = null
                    set T_DST = dst
                    set T_TIMEOUT = timeout
