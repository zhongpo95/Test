library AOE initializer Init requires MonoEvent, DamageEffect2, Tick
    globals
        constant key E_AOE
    endglobals

    //색상 우선순위는 안전(초록) > 특수 상태이상(파랑) > 넉백·에어본·스턴(노랑) > 피해(빨강)
    ...
    private function AOE_Tick takes nothing returns nothing
        ...
        if st.stopFlag != true and st.timenow >= st.time then
            if st.eft != 0 then
    ...
    /*types 0=red 1=blue 2=yellow 3=green
        초록은 안전구역
        파랑은 감전이나 암흑 같은 특수 상태이상이 포함된 공격
        노랑은 넉백, 에어본, 스턴이 포함된 공격
