# 만화경 v0.175 헤라 호환 시험본 v7

제공된 `万华镜v0.175.w3x`를 보존하고 별도 `Hera_Wanhua_0175_v7.w3x`를 만든다. 원본 제작자 SDK 없이 맵 내부 호출부를 분석해 만든 **초기 호환 시험본**이다. 실제 헤라 로딩, 영웅 선택, 전투 및 멀티플레이 성공은 아직 확인하지 않았다.
...
오류와 지원하지 않는 호출은 게임 폴더의 `Logs/Hera_Wanhua_v7_p1.txt`에 기록한다. 플레이어 번호에 따라 `p2`, `p3` 등으로 바뀐다. 초기화 상태에도 시험본과 서버 저장 미지원 안내가 표시된다. `initialized; gameplay unverified`는 Lua 초기화 반환 상태이며 게임 정상 작동 판정이 아니다. FDF 로드 네이티브는 반환값이 없으므로 템플릿이 실제로 화면에 적용됐는지는 게임에서 확인해야 한다.
...
python tools/wanhuajing-hera/build.py 'C:/maps/万华镜v0.175.w3x' 'C:/build/Hera_Wanhua_0175_v7.w3x' --stormlib 'C:/tools/StormLib.dll'
```
...
python tools/wanhuajing-hera/check_pack.py --source 'C:/maps/万华镜v0.175.w3x' --output 'C:/build/Hera_Wanhua_0175_v7.w3x' --stormlib 'C:/tools/StormLib.dll' --report 'C:/build/staging/build-report.json'
```
...
v7의 실제 게임 및 화면 검사는 미실시다. v6는 사용자가 UI 표시와 초록 화면을 확인했으며, 실행 로그의 후속 오류가 남아 있었다. 첫 확인 순서는 맵 선택·로딩, 초기 화면 버튼과 영웅 선택, 이동·기본 스킬, 2인 접속 및 동기화다.
