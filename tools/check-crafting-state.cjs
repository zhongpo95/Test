...
const cardNames=['StoneCurrentText','StoneStageText','StoneSlot','StoneServerReady','StoneRefresh','StoneStart','StoneBegin','StoneSetOpen','ClickButton','ButtonWork'];
const card=model('UI/UI_Stone.j',cardNames),c=card.env;
assert.equal(c.StoneTemporaryMode,true);
c.StoneTemporaryMode=false;
for(const [count,level] of [[0,0],[5,0],[6,1],[7,2],[8,2],[9,3],[10,4]])assert(c.StoneCurrentText(count,false).includes('Lv '+level));
for(const [count,level] of [[0,0],[4,0],[5,1],[6,1],[7,2],[9,2],[10,3]]){
  const label=c.StoneCurrentText(count,true);assert(label.includes('Lv '+level));assert(!label.includes('%'));
}
...
for(const threshold of [5,6,7,9,10]){
  ...
  assert.equal(c.StoneStageText(threshold,threshold,'6','|cff168eae'),'|cff168eae✓ 6|r');
}
assert.equal(c.StoneStageText(7,7,'4','|cffc85e7b'),'|cffc85e7b✓ 4|r');
