...
| 워크래프트 실제 실행 | **사용자 실행으로 확인.** JNLoader OpenGL 실행 후 맵 로딩과 `-video` 영상 출력 성공. 게임 로그에서 OpenGL 4.6.0 NVIDIA 616.92 감지, 맵 MP4 열기, 240프레임 디코딩 완료 확인. |
| 게임 내 추가 검증 | **미수행.** 재생 종료 후 화면 복원, 수동 닫기·재재생, 창 전환·해상도 변경 및 다른 플러그인과의 장시간 공존은 별도 확인 필요. |
