...
워크래프트를 **JNLoader와 OpenGL 모드**로 실행했을 때, 맵에 포함된 MP4/WebM 영상을 화면 중앙에서 재생한다. 현재 시제품은 **640×360, 30fps, 무음**이다. 웹페이지 표시와 음성 재생은 구현 범위에 포함하지 않는다.

## 구성과 동작

- `Plugin.cs`는 JN의 `IPlugin`으로 로드되고 영상용 네이티브 3개를 등록한다. 맵 자원은 JN의 Storm API로 읽는다.
- `VideoPlayer.cs`는 별도 FFmpeg 프로세스에서 BGRA 영상을 받아 고정 크기 버퍼 두 개로 전달한다. FFmpeg 창은 표시하지 않는다.
- `OpenGlOverlay.cs`는 한국용 Game.dll이 사용하는 `wglSwapLayerBuffers`와 일반 `SwapBuffers`에 EasyHook을 연결한다. 중첩 호출에서는 한 번만 그린다. 현재 OpenGL 컨텍스트에서 영상을 그리고 그래픽 상태를 복원한다. 맵 종료와 닫기 요청에서 디코더를 정리하고, 텍스처는 해당 OpenGL 컨텍스트의 다음 화면 교체 때 해제한다.
...
4. `-video`를 입력한다. 중앙에 색상표와 움직이는 무늬·시간이 들어 있는 8초 무음 영상이 나와야 한다.
5. 재생 중 `-videoclose`로 즉시 닫거나, 8초 뒤 자동으로 닫히는지 확인한다. `-video`로 다시 재생할 수 있다.
...
| 독립 실행 | 숨겨진 별도 시험 창의 NVIDIA OpenGL 4.6 컨텍스트에서 실제 디코딩·그리기·SwapBuffers 및 wglSwapLayerBuffers 연결 시험 통과. |
...
| 시각 확인 | 독립 시험 프로그램의 OpenGL 화면과 사용자가 제공한 실제 Warcraft 화면에서 테스트 영상 출력 확인. 게임 화면에는 영상의 1.467초 지점이 표시됨. |
| MPQ 보존 | 기존 905개 중 902개 동일. war3map.j·war3map.imp·war3map.wts 수정, MP4 1개 추가. 원본 SHA-256 유지. |
| 워크래프트 실제 실행 | **사용자 실행으로 확인.** JNLoader OpenGL 실행 후 맵 로딩과 `-video` 영상 출력 성공. 게임 로그에서 OpenGL 4.6.0 NVIDIA 616.92 감지, 맵 MP4 열기, 240프레임 디코딩 완료 확인. 사용자가 명령 재입력 시 재재생과 재생 종료 후 영상이 정상적으로 사라지는 동작도 확인함. |
| 게임 내 추가 검증 | **미수행.** 수동 닫기, 창 전환·해상도 변경 및 다른 플러그인과의 장시간 공존은 별도 확인 필요. |
