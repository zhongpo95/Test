...
function ModuloInteger(dividend, divisor)
  ...
  local modulus = dividend - quotient * divisor
  if modulus < 0 then
...
function TriggerExecuteBJ(trig, checkConditions)
  if checkConditions and not TriggerEvaluate(trig) then
    return false
  end
  TriggerExecute(trig)
  return true
...
function QueuedTriggerRemoveBJ(trig)
  local index, trigIndex, trigExecuted
...
function TerrainDeformationRippleBJ(duration, limitNeg, where, startRadius, endRadius, depth, wavePeriod, waveWidth)
  local spaceWave, timeWave, radiusRatio
...
function TerrainDeformationWaveBJ(duration, source, target, radius, depth, trailDelay)
  local distance, dirX, dirY, speed
...
function EnumItemsInRectBJ(r, actionFunc)
  -- 호출별 매개변수를 캡처하여 엔진 콜백 재사용 여부를 일정하게 유지한다.
...
function DelayedSuspendDecayStopAnimEnum()
  ...
  if GetUnitState(enumUnit, UNIT_STATE_LIFE) <= 0 then
    SetUnitTimeScale(enumUnit, 1.0E-4)
  end
end
...
function ReplaceUnitBJ(whichUnit, newUnitId, unitStateMethod)
  ...
  local newUnit, wasHidden, index, indexItem, oldRatio
  if oldUnit == nil then
  ...
  if unitStateMethod == bj_UNIT_STATE_METHOD_RELATIVE then
    ...
    if 0 < GetUnitState(oldUnit, UNIT_STATE_MAX_MANA) and 0 < GetUnitState(newUnit, UNIT_STATE_MAX_MANA) then
  ...
  elseif unitStateMethod == bj_UNIT_STATE_METHOD_ABSOLUTE then
    ...
    if 0 < GetUnitState(newUnit, UNIT_STATE_MAX_MANA) then
...
function ChangeElevatorWallBlocker(x, y, facing, open)
  ...
  if open then
    if 0 < GetDestructableLife(blocker) then
...
function ChangeElevatorWalls(open, walls, d)
  ...
  if (walls == bj_ELEVATOR_WALL_TYPE_ALL or walls == bj_ELEVATOR_WALL_TYPE_EAST) and not NearbyElevatorExists(x + distToNeighbor, y) then
    ChangeElevatorWallBlocker(x + distToBlocker, y, 0, open)
  end
  if (walls == bj_ELEVATOR_WALL_TYPE_ALL or walls == bj_ELEVATOR_WALL_TYPE_NORTH) and not NearbyElevatorExists(x, y + distToNeighbor) then
    ChangeElevatorWallBlocker(x, y + distToBlocker, 90, open)
  end
  if (walls == bj_ELEVATOR_WALL_TYPE_ALL or walls == bj_ELEVATOR_WALL_TYPE_SOUTH) and not NearbyElevatorExists(x, y - distToNeighbor) then
    ChangeElevatorWallBlocker(x, y - distToBlocker, 90, open)
  end
  if (walls == bj_ELEVATOR_WALL_TYPE_ALL or walls == bj_ELEVATOR_WALL_TYPE_WEST) and not NearbyElevatorExists(x - distToNeighbor, y) then
    ChangeElevatorWallBlocker(x - distToBlocker, y, 0, open)
  end
end
...
function Group_CountsEnum()
  bj_groupCountUnits = bj_groupCountUnits + 1
end

function Group_Counts(g)
  local wantDestroy = bj_wantDestroyGroup
  bj_wantDestroyGroup = false
  bj_groupCountUnits = 0
  ForGroup(g, Group_CountsEnum)
  if wantDestroy then
    DestroyGroup(g)
  end
...
function GetRandomSubGroupEnum()
  if bj_randomSubGroupWant > 0 and (bj_randomSubGroupWant >= bj_randomSubGroupTotal or GetRandomReal(0, 1) < bj_randomSubGroupChance) then
...
function GetRandomSubGroup(count, sourceGroup)
  ...
  bj_randomSubGroupTotal = Group_Counts(sourceGroup)
...
function PlayersAreCoAllied(playerA, playerB)
  ...
  if GetPlayerAlliance(playerA, playerB, ALLIANCE_PASSIVE) and GetPlayerAlliance(playerB, playerA, ALLIANCE_PASSIVE) then
    return true
  end
  return false
end
...
function ShareEverythingWithTeamAI(whichPlayer)
  ...
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
    if PlayersAreCoAllied(whichPlayer, indexPlayer) and whichPlayer ~= indexPlayer and GetPlayerController(indexPlayer) == MAP_CONTROL_COMPUTER then
...
function TransmissionFromUnitWithNameBJ(toForce, whichUnit, unitName, soundHandle, message, timeType, timeVal, wait)
  ...
  if wait and 0 < bj_lastTransmissionDuration then
...
function CinematicFadeBJ(fadetype, duration, tex, red, green, blue, trans)
  ...
  elseif fadetype == bj_CINEFADETYPE_FADEIN then
    ...
    if fadetype == bj_CINEFADETYPE_FADEOUTIN and 0 < duration then
      AbortCinematicFadeBJ()
...
function SetMissionAvailableBJ(available, missionIndex)
  local campaignNumber = math.floor(missionIndex / 1000)
...
function GetFadeFromSeconds(seconds)
  if seconds ~= 0 then
    return math.floor(128 / R2I(seconds))
  end
  return 10000
end
...
function BlightGoldMineForPlayerBJ(goldMine, whichPlayer)
  local mineX, mineY, mineGold, newMine
...
function MeleeStartingResources()
  local index, indexPlayer, v, startingGold, startingLumber
  v = VersionGet()
...
function ReducePlayerTechMaxAllowed(whichPlayer, techId, limit)
  ...
  if oldMax < 0 or limit < oldMax then
...
function MeleeClearExcessUnit()
  ...
  elseif owner == PLAYER_NEUTRAL_PASSIVE and not IsUnitType(theUnit, UNIT_TYPE_STRUCTURE) then
    RemoveUnit(GetEnumUnit())
  end
end
...
function MeleeClearExcessUnits()
  local index, locX, locY, indexPlayer
  index = 0
  for _ in _loop_() do
    indexPlayer = Player(index)
    if GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING then
...
function MeleeStartingUnitsHuman(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
  ...
  local nearestMine, nearMineLoc, heroLoc, peonX, peonY, townHall
  if doPreload then
...
function MeleeStartingUnitsOrc(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
  local useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
  local unitSpacing = 64.0
  local nearestMine, nearMineLoc, heroLoc, peonX, peonY
  if doPreload then
...
function MeleeStartingUnitsUndead(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
  local useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
  local unitSpacing = 64.0
  local nearestMine, nearMineLoc, nearTownLoc, heroLoc, peonX, peonY, ghoulX, ghoulY
  if doPreload then
...
function MeleeStartingUnitsNightElf(whichPlayer, startLoc, doHeroes, doCamera, doPreload)
  ...
  local nearestMine, nearMineLoc, wispLoc, heroLoc, peonX, peonY, tree
  if doPreload then
...
function MeleeStartingUnits()
  local index, indexPlayer, indexStartLoc, indexRace
...
function MeleeStartingAI()
  local index, indexPlayer, indexRace
  index = 0
  for _ in _loop_() do
    indexPlayer = Player(index)
    if GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING then
...
function MeleePlayerIsOpponent(playerIndex, opponentIndex)
  ...
  if GetPlayerAlliance(thePlayer, theOpponent, ALLIANCE_PASSIVE) and GetPlayerAlliance(theOpponent, thePlayer, ALLIANCE_PASSIVE) and GetPlayerState(thePlayer, PLAYER_STATE_ALLIED_VICTORY) == 1 and GetPlayerState(theOpponent, PLAYER_STATE_ALLIED_VICTORY) == 1 then
    return false
  end
  return true
end
...
function MeleeGetAllyStructureCount(whichPlayer)
  local playerIndex, buildingCount, indexPlayer
  buildingCount = 0
  playerIndex = 0
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
...
function MeleeGetAllyCount(whichPlayer)
  local playerIndex, playerCount, indexPlayer
  playerCount = 0
  playerIndex = 0
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
...
function MeleeGetAllyKeyStructureCount(whichPlayer)
  local playerIndex, indexPlayer, keyStructs
  keyStructs = 0
  playerIndex = 0
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
...
function MeleeCheckForVictors()
  local playerIndex, opponentIndex
...
function MeleeCheckForLosersAndVictors()
  ...
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
    if not bj_meleeDefeated[playerIndex] and not bj_meleeVictoried[playerIndex] and 0 >= MeleeGetAllyStructureCount(indexPlayer) then
...
function MeleeExposeAllPlayers()
  local playerIndex, indexPlayer, playerIndex2, indexPlayer2
  local toExposeTo = CreateForce()
  playerIndex = 0
  for _ in _loop_() do
    indexPlayer = Player(playerIndex)
    ...
    for _ in _loop_() do
      ...
      if playerIndex ~= playerIndex2 and not PlayersAreCoAllied(indexPlayer, indexPlayer2) then
...
function MeleeCrippledPlayerTimeout()
  ...
  local playerIndex, exposedPlayer
  playerIndex = 0
  for _ in _loop_() do
    if bj_crippledTimer[playerIndex] == expiredTimer and true then
      break
    end
    playerIndex = playerIndex + 1
    if playerIndex == bj_MAX_PLAYERS then
...
function MeleePlayerIsCrippled(whichPlayer)
  ...
  return 0 < allyStructures and allyKeyStructures <= 0
end

function MeleeCheckForCrippledPlayers()
  local playerIndex, indexPlayer
  local crippledPlayers = CreateForce()
  local isNowCrippled, indexRace
  ...
  for _ in _loop_() do
    ...
    elseif bj_playerIsCrippled[playerIndex] and not isNowCrippled then
      ...
      if GetLocalPlayer() == indexPlayer then
        TimerDialogDisplay(bj_crippledTimerWindows[playerIndex], false)
        if 0 < MeleeGetAllyStructureCount(indexPlayer) then
...
function MeleeTournamentFinishNowRuleA(multiplier)
  ...
  local teamCount, index, indexPlayer, index2, indexPlayer2, bestTeam, bestScore, draw
  index = 0
  for _ in _loop_() do
    indexPlayer = Player(index)
  ...
  if teamCount ~= 0 then
    ...
    index = 0
    for _ in _loop_() do
      if bestScore < teamScore[index] then
        bestTeam = index
    ...
    index = 0
    for _ in _loop_() do
      if index ~= bestTeam and bestScore < multiplier * teamScore[index] then
        draw = true
      end
      index = index + 1
      if index == teamCount then
...
function InitBlizzardGlobals()
  local index, userControlledPlayers, v
...
function PerformStockUpdates()
  local pickedItemId, pickedItemType
