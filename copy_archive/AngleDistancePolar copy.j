 ...
 *   3. GetRandomDirectionDeg()가 너무 길어서 AngleRandom() & AngleRadRandom() 추가
 * 
 * [각도 & 거리]
 ...
 * 
 * [각도]
 *   AngleRandom() //무작위 각도(0~360)를 가져옵니다.
 *   AngleRadRandom() //무작위 호도(0.0~6.28)를 가져옵니다.
 *   AnglePBP //0~360도 체계를 사용합니다.
 *   AnglePBW //0~360도 체계를 사용합니다.
 *   AngleWBP //0~360도 체계를 사용합니다.
 *   AngleWBW //0~360도 체계를 사용합니다.
 *   AngleRadPBP //라디안 값(0.0~6.28)을 반환합니다.
 *   AngleRadPBW //라디안 값(0.0~6.28)을 반환합니다.
 *   AngleRadWBP //라디안 값(0.0~6.28)을 반환합니다.
 *   AngleRadWBW //라디안 값(0.0~6.28)을 반환합니다.
 *   Angletrans( 원본 각도, 목표 각도, 유도율 ) //원본 각도에 유도율을 더한 각도를 반환합니다. 목표 각도를 넘지 않습니다.
 *     예시))Angletrans( 유도미사일의 현재 각도, 유도미사일과 적 사이 각도, 1 )
 *   AngleUTA //유닛이 각도(0~360)를 바라보게 합니다.
 *   AngleUTP //유닛이 지점을 바라보게 합니다.
 *   AngleUTW //유닛이 위젯을 바라보게 합니다.
 *   AngleRadUTA //유닛이 라디안 각(0.0~6.28)을 바라보게 합니다.
 *   
 * [거리]
 *   DistancePBP
 *   DistancePBW
 ...
 *   
 * [오프셋]
 *   PolarX(거리, 각도) //원점(0,0)으로부터 (거리), (각도)만큼의 위치한 좌표의 X값을 반환합니다.
 *   PolarY(거리, 각도) //원점(0,0)으로부터 (거리), (각도)만큼의 위치한 좌표의 Y값을 반환합니다.
 *   Polar.RadX(거리, 라디안각)
 *   Polar.RadY(거리, 라디안각)
 ...
 *
 * [오프셋-각도]
 *   PolarAngleUTA(유닛, 거리, 각도) //Polar.UTA + AngleUTA
 *   PolarAngleUTP(유닛, 거리, X, Y) //Polar.UTP + AngleUTP
 *   PolarAngleUTW(유닛, 거리, 대상) //Polar.UTW + AngleUTW
 *   PolarAngleRadUTA(유닛, 거리, 라디안각) //Polar.RadUTA + AngleRadUTA
 *
 * [베타 기능 - 좌표]
