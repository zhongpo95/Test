...
// ============================================================================
// 카드부여 조합별 피해 증가 및 목표 획득 확률 참고표. (2026-09-14)
//
// 계산 기준.
// - 이로운 효과 2줄과 균열 1줄에 각각 10회 세공하며, 시작 성공 확률은 75%이다.
// - 어느 줄이든 성공하면 다음 확률이 10%p 내려가고, 실패하면 10%p 올라간다.
//   성공 확률 범위는 25~75%이며, 매 세공의 난수는 독립적인 균등분포로 가정한다.
// - 성공 수에 따른 줄당 피해 증가는 0~5회 0%, 6회 6%, 7~8회 7.5%,
//   9회 10.5%, 10회 12%이다. 두 줄은 곱연산으로 적용된다.
//   합산 피해 증가(%) = ((1 + 첫 줄 증가율) * (1 + 둘째 줄 증가율) - 1) * 100.
// - 피해 증가는 표기된 조합 자체의 값이며, 균열 및 다른 장비의 패널티는 제외한다.
//
// 확률 및 평균 제작량의 의미.
// - 목표 A/B는 두 이로운 줄의 성공 수가 각각 A/B 이상이거나 B/A 이상인 경우이다.
//   예를 들어 9/7 목표에는 7/9, 9/8, 10/7 등도 포함되며, 정확히 9/7만 뜻하지 않는다.
// - 획득 확률은 균열 성공 수를 제한하지 않고, 각 목표의 달성 확률을 최대화하도록
//   매 세공 결과에 따라 다음 줄을 선택했을 때의 카드 1장당 이론상 최대 확률이다.
// - 남은 줄별 세공 횟수, 이로운 줄별 성공 수, 현재 확률을 상태로 둔 동적 계획법으로
//   각 선택의 성공/실패 후 목표 달성 확률을 가중 평균하고 최댓값을 택해 계산했다.
// - 평균 제작량은 독립적으로 재제작할 때 첫 목표 달성까지 필요한 카드 수의 기댓값이다.
//   평균 제작량 = 1 / (획득 확률(%) / 100). 해당 수량 안의 획득을 보장하지 않는다.
// - 목표마다 최적 전략이 다르고 상위 결과가 중복 포함되므로 행별 확률을 합산하지 않는다.
//   특히 8/8 행은 9/7을 노리는 전략에서 8/8이 나오는 확률을 나타내지 않는다.
// - 8회에는 추가 보상이 없어 8/8과 7/7의 피해 증가는 같다. 8/8은 9/7 목표 미달이다.
//
// 조합     | 조합 피해 증가 | 목표 이상 획득 확률 | 평균 제작량
// ---------+----------------+---------------------+---------------------
// 6/6      |       12.3600% |    39.714920607191% |               2.52장
// 7/6      |       13.9500% |    24.832961702627% |               4.03장
// 7/7      |       15.5625% |     5.038612923984% |              19.85장
// 8/6      |       13.9500% |     5.985864812261% |              16.71장
// 8/7      |       15.5625% |     1.455033657913% |              68.73장
// 9/6      |       17.1300% |     0.670777381624% |             149.08장
// 8/8      |       15.5625% |     0.105567231721% |             947.26장
// 9/7      |       18.7875% |     0.120564275780% |             829.43장
// 10/6     |       18.7200% |     0.024903557139% |           4,015.49장
// 9/8      |       18.7875% |     0.011721973614% |           8,530.99장
// 10/7     |       20.4000% |     0.003598267562% |          27,791.15장
// 9/9      |       22.1025% |     0.000329039397% |         303,914.97장
// 10/8     |       20.4000% |     0.000297372633% |         336,278.42장
// 10/9     |       23.7600% |     0.000013919588% |       7,184,120.59장
// 10/10    |       25.4400% |     0.000000161563% |     618,952,522.25장
//
// 위 값은 현재 규칙의 계산값이며, 실제 게임의 관측 통계가 아니다.
// 세공 확률, 시도 횟수, 효과 단계 또는 System/StatsSetting.j의 배율 변경 시 재계산한다.
// ============================================================================
