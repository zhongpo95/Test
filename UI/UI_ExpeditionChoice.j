...
library UIExpeditionChoice initializer Init requires UIExpeditionCommon
    ...
    private function RenderChoice takes integer pid returns nothing
        ...
        if ExpState == EXP_START then
            ...
            call SetCard(ChoiceCards[2], "치명 +200", "포인트를 사용하지 않고 치명이 증가합니다.", "고정 능력치", "BTNClawsOfAttack", 1, "선택", true)
            call SetCard(ChoiceCards[3], "신속 +200", "포인트를 사용하지 않고 신속이 증가합니다.", "고정 능력치", "BTNBootsOfSpeed", 1, "선택", true)
            call SetCard(ChoiceCards[4], "골드 +200", "이번 원정의 상점에서 사용할 골드를 얻습니다.", "골드", "BTNChestOfGold", 1, "선택", true)
            call SetCard(ChoiceCards[5], "일반 각인 +2", "무작위 일반 각인을 2레벨 얻습니다.|n패널티 없음", "각인", "BTNPeriapt", 1, "선택", true)
            if ExpStartTwoCards[pid] then
                call SetCard(ChoiceCards[6], "일반 카드 2장", "무작위 일반 카드를 2장 얻습니다.", "일반 카드", "BTNTome", 1, "선택", true)
            else
                call SetCard(ChoiceCards[6], ExpCardName(ExpStartCard[pid]), ExpCardText(ExpStartCard[pid]), "일반 카드", "BTNTome", 1, "선택", true)
            endif
            call SetCard(ChoiceCards[7], "희귀 카드 1장", "무작위 희귀 카드를 1장 얻습니다.", "희귀 카드", "BTNTomeOfRetraining", 2, "선택", true)
        endif
    endfunction
    ...
    private function Render takes nothing returns nothing
        ...
        elseif ExpState == EXP_REWARD and ExpEventDeadline[pid] == 0 then
            ...
            call RenderChoice(pid)
            call SetCard(ChoiceCards[8], "스탯 5포인트", "스탯 배분에 사용할 포인트를 얻습니다.|n|n40포인트 초과분은 포인트당 20골드로 받습니다.", "스탯", "BTNManual", 1, "선택", true)
            call SetCard(ChoiceCards[9], "각인 획득", ArcanaText[ExpArcanaA[pid]] + " +" + I2S(ExpArcanaLevel[pid]) + "|n" + ArcanaText[ExpArcanaB[pid]] + " +1|n|n|cff98284d" + ArcanaText[ExpPenalty[pid]] + " +" + I2S(ExpArcanaLevel[pid]) + "|r", "각인", "BTNPeriapt", 2, "선택", true)
            if ExpEventCandidate[pid] == 0 then
                call SetCard(ChoiceCards[10], "골드 획득", I2S(ExpGradeGold(ExpEventGrade[pid])) + "골드를 얻습니다.", ExpEventGradeName(ExpEventGrade[pid]), "BTNChestOfGold", ExpEventGrade[pid], "선택", true)
            else
                set name = JNStringSplit(ExpEventText(ExpEventCandidate[pid], 1), " · ", 0)
                call SetCard(ChoiceCards[10], name, "사건에 진입한 뒤 제시되는 보상 중 하나를 선택합니다.", ExpEventGradeName(ExpEventGrade[pid]) + " 사건", "BTNScroll", ExpEventGrade[pid], "진입", true)
            endif
        endif
        set i = 1
        loop
            exitwhen i > 10
            call DzFrameShow(CardButton[ChoiceCards[i]], (ExpState == EXP_START and i <= 7) or (ExpState == EXP_REWARD and i >= 8))
            set i = i + 1
        endloop
        ...
        if ExpState == EXP_SHOP then
            call ExpUIText(ShopTitle, "상점    " + I2S(ExpGold[pid]) + " G")
            ...
            call ExpUISetButton(ShopButtons[4], "치명 +100|n" + I2S(price) + " G", not ExpStatBought[pid] and ExpGold[pid] >= price)
            call ExpUISetButton(ShopButtons[5], "신속 +100|n" + I2S(price) + " G", not ExpStatBought[pid] and ExpGold[pid] >= price)
            set i = 6
            loop
                exitwhen i > 8
                set price = 100
                ...
                set price = ExpShopPrice(pid, price)
                set name = JNStringSplit("회복|공격|무적", "|", i - 6) + " 물약 " + I2S(ExpPotionBought[ExpKey(pid, i - 5)]) + "/2|n" + I2S(price) + " G"
    ...
    private function Build takes nothing returns nothing
        ...
        loop
            exitwhen i > 7
            set ChoiceCards[i] = MakeCard(ChoiceRoot, i, 0.017 + (i - 1) * 0.108, 0.038, 0.098, 0.292)
            set i = i + 1
        endloop
        set i = 1
        loop
            exitwhen i > 3
            set ChoiceCards[i + 7] = MakeCard(ChoiceRoot, i, 0.048 + (i - 1) * 0.233, 0.038, 0.218, 0.292)
            set i = i + 1
        endloop
