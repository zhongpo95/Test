library UIItem initializer Init requires DataItem, StatsSet, UIShop, ITEM
    ...
    private function ClickStItemButton takes nothing returns nothing
        ...
        if items != "0" then
                ...
                //아이템을 들고있으면 자리바꿈
                    ...
                    if F_ItemClickNumber < 50 then
                        if (selectnumber-10000) < 50 then
                            ...
                            call DzFrameSetTexture(F_ItemButtonsBackDrop[F_ItemClickNumber], GetItemArt(items), 0)
                            call StashSave(pid:PLAYER_DATA, "슬롯"+sn+".아이템"+I2S(F_ItemClickNumber), items)
                    ...
                    elseif (F_ItemClickNumber - 10000) < 100 and F_ItemClickNumber >= 10000 then
                        if (selectnumber-10000) < 100 then
                            set items2 = StashLoad(pid:PLAYER_DATA, "창고"+I2S(F_ItemClickNumber-10000), "0")
                            call AddStItem.evaluate(pid, F_ItemClickNumber-10000, items)
                            call AddStItem.evaluate(pid, selectnumber-10000, items2)
                        endif
            ...
            if i == 1 then
                ...
                if j < 50 then
                    //창고에서 제거
                    call DzFrameSetTexture(F_Storage_ButtonsBackDrop[F_ItemClickNumber - 10000], "UI_Inventory.blp", 0)
                    call DzFrameShow(UI_Tip, false)
                    call StashRemove(pid:PLAYER_DATA, "창고"+I2S(F_ItemClickNumber - 10000))
                    call DzFrameShow(F_Storage_ButtonLock[F_ItemClickNumber - 10000], false)
                    //인벤에 추가
                ...
                //템을 들고있으면 자리바꿈
                    ...
                    if F_ItemClickNumber < 50 then
                        if selectnumber < 50 then
                            ...
                            call DzFrameSetTexture(F_ItemButtonsBackDrop[F_ItemClickNumber], GetItemArt(items), 0)
                            call StashSave(pid:PLAYER_DATA, "슬롯"+sn+".아이템"+I2S(F_ItemClickNumber), items)
                            call DzFrameSetTexture(F_ItemButtonsBackDrop[selectnumber], GetItemArt(items2), 0)
                            call StashSave(pid:PLAYER_DATA, "슬롯"+sn+".아이템"+I2S(selectnumber), items2)
                        endif
                        call DzFrameShow(F_PickUp, false)
                        set F_ItemClickNumber = 200
                    //기타
            ...
            if i == 1 then
                if itemty == 7 then
                    ...
                    if etyid1 == 0 then
                        ...
                        call StashRemove(pid:PLAYER_DATA, "슬롯"+sn+".아이템"+I2S(F_ItemClickNumber))
                    ...
                    elseif etyid2 == 0 then
                        ...
                        call StashSave(pid:PLAYER_DATA, "슬롯"+sn+".아이템"+I2S(F_ItemClickNumber), items2)
    ...
    private function CreateStItemBackDrop takes integer types, real x, real y returns nothing
        set F_Storage_Buttons[types]=DzCreateFrameByTagName("BUTTON", "", F_Storage_EQButtonsShow, "ScoreScreenTabButtonTemplate", 0)
        ...
        set F_Storage_ButtonsBackDrop[types]=DzCreateFrameByTagName("BACKDROP", "", F_Storage_Buttons[types], "", 0)
        ...
        set F_Storage_ButtonsBackDrop[types+50]=DzCreateFrameByTagName("BACKDROP", "", F_Storage_Buttons[types+50], "", 0)
    ...
    private function CreateItemButton takes integer types, real x, real y returns nothing
        set F_ItemButtons[types]=DzCreateFrameByTagName("BUTTON", "", F_EQButtonsShow, "ScoreScreenTabButtonTemplate", 0)
        ...
        set F_ItemButtonsBackDrop[types]=DzCreateFrameByTagName("BACKDROP", "", F_ItemButtons[types], "", 0)
        ...
        set F_ItemButtons[types+50]= DzCreateFrameByTagName("BUTTON", "", F_ETCButtonsShow, "ScoreScreenTabButtonTemplate", 0)
        ...
        set F_ItemButtonsBackDrop[types+50]=DzCreateFrameByTagName("BACKDROP", "", F_ItemButtons[types+50], "", 0)
