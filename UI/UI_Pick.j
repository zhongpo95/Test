...
library UIPick initializer Init requires UIHP, UISkillLevel, UIItem, Daily, FrameCount, ItemPickUp, DzAPIHardware
    ...
    private function PickSelectedSkinNumber takes integer heroNumber returns integer
        if heroNumber == 2 then
            return PickSkinNumber
        endif
        return 1
    endfunction

    private function ApplyPickHeroSkinModel takes integer heroNumber, integer skinNumber, unit target returns nothing
        if heroNumber == 2 and skinNumber == 2 then
            call DzSetUnitModel(target, "Narmaya_Swimsuit.mdx")
        endif
    endfunction
    ...
    private function PickMaxScrollRow takes nothing returns integer
        return PickMaxScrollOffset() / PickCardColumnCount
    endfunction
    ...
    private function SetPickScrollOffset takes integer pid, integer value returns nothing
        local integer maxOffset = PickMaxScrollOffset()
        set value = ((value + PickCardColumnCount / 2) / PickCardColumnCount) * PickCardColumnCount
        if value < 0 then
            set value = 0
    ...
    private function LoadPickF takes nothing returns nothing
        ...
        call ApplyPickHeroSkinModel(SlotHero, SkinNumber, MainUnit[pid])
