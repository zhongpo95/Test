...
library UIPick initializer Init requires UIHP, UISkillLevel, UIItem, UIMainQuest, Daily, FrameCount, ItemPickUp, DzAPIHardware
    ...
    private function PickValidSkinNumber takes integer heroNumber, integer skinNumber returns integer
        if skinNumber >= 1 and skinNumber <= PickHeroSkinCount(heroNumber) then
            return skinNumber
        endif
        return 1
    endfunction
    ...
    private function RefreshPickConfirm takes integer pid returns nothing
        ...
        if not PickSkinUnlocked(pid, SHNumber, PickSkinNumber) then
            call DzFrameShow(FP_SelectBBD, false)
            return
        endif
